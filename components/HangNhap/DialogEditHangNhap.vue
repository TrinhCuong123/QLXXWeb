<template>
  <Dialog v-model:visible="internalVisible" :style="{ width: '800px' }" :header="'Cập nhật nhập hàng'" :modal="true"
...
watchEffect(() => {
  if (internalData.value.id) {
    id.value = internalData.value.id;
    tenLanNhap.value = internalData.value.ten_lannhap;
    ngayNhap.value = DateUtils.formatDate(internalData.value.ngay_nhaphang);
  }
});
...
const onSubmit = handleSubmit(() => {
  ...
  confirm.require({
    message: 'Bạn có muốn cập nhật hàng nhập không?',
    header: 'Cập nhật',
    ...
    accept: () => {
      NhapHangService.update(nhapHangData)
        .then((res) => {
          if (res) {
            emit('updateModel');
            handleHideModal();
