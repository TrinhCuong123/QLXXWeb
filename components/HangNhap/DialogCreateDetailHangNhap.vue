<template>
  <Dialog v-model:visible="internalVisible" :style="{ width: '1000px' }" :header="'Thêm hàng nhập'" :modal="true"
    ...
    <div class="gap-4 grid grid-cols-1 sm:grid-cols-2">
      <div class="min-w-40">
        ...
        <Select id="maPhuTung" v-model="maPhuTung" :options="internalPhuTung" option-label="ma_phutung" option-value="id"
          placeholder="Chọn tên phụ tùng" size="large" show-clear filter class="mr-2 w-full h-[42.72px]" />
      </div>
      <div class="min-w-40">
...
const onSubmit = handleSubmit(() => {
  ...
  confirm.require({
    ...
    accept: () => {
      ChiTietNhapHangService.insert(customerData)
          .then((res) => {
            if (res) {
              emit('updateModel');
              handleHideModal();
